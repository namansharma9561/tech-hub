                                ...
                                <li key={index} >
                                    {
                                        element.title === "Catalog" ? (<div className=' flex items-center group relative cursor-pointer'>
                                            ...
                                            <div className="invisible absolute left-1/2 top-full z-10 flex w-48 flex-col rounded-md bg-richblack-700 p-2 text-richblack-100 opacity-0 transition-transform duration-150 group-hover:visible group-hover:opacity-100 group-hover:translate-y-2 transform -translate-x-1/2">
                                                <div className="absolute left-1/2 top-0 h-3 w-3 -translate-x-1/2 -translate-y-1/2 rotate-45 bg-richblack-700"></div>
                                                {sublinks?.length > 0 &&
                                                    sublinks.map((element, index) => (
                                                    <Link
                                                        to={`/catalog/${element?.name}`}
                                                        key={index}
                                                        className="px-3 py-1 hover:bg-richblack-800 rounded-md text-sm"
